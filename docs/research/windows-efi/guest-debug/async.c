...
volatile u64 emu_count, emu_reason, emu_pcs[64], emu_gp[32], emu_ranges[64], emu_nrange, emu_vector;
...
static EFI_STATUS trace_entry(EFI_HANDLE child) {
    ...
    emit("vector slot "); emit_u64(emu_vector,0); emit(" (slot%4: 0=sync 1=IRQ 2=FIQ 3=SError)\n");
    if (emu_reason==4) emit("ESR is not interpreted for an asynchronous vector\n");
    emit("emulated reads "); emit_u64(emu_count,0); emit(" stop reason "); emit_u64(emu_reason,0); emit(" (1=64-read-limit 2=different-synchronous-exception 3=outside-executable-image 4=asynchronous-vector)\n");
