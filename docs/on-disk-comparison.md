...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  dry-runs, image-preparation dry-runs with skipped-worker diagnostics and controls, operations dashboard summaries, worker/assignment inventory, maintenance dry-runs, controls, skip diagnostics, and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, metering, and GUI events, plus persisted
...
switches with capability-constrained hosted lifecycle, placement dry-runs, image-preparation dry-runs with skipped-worker diagnostics and controls, operations dashboard summaries, worker/assignment inventory, maintenance dry-runs, controls, skip diagnostics, and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, metering, and GUI events, persisted per-resource sandbox usage
