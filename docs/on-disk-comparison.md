...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  replay rejection, worker quarantine for suspect hosts, worker drain for hosted sandbox maintenance, idle/drained
...
expired-lease reconciliation, controller-side worker cordon/quarantine/drain/decommission
