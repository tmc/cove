...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  with per-namespace admission caps, queue priority, queue-deadline expiry, per-run timeout policy, bounded automatic retry, sandbox-scoped exec/control cancellation during stop/delete/restart/drain, client-visible returned canceled assignment ids, and enforced modify leases
...
handles over fork-run assignments with per-namespace admission caps, queue priority, queue-deadline expiry, per-run timeout policy, bounded automatic retry, sandbox-scoped exec/control cancellation during stop/delete/restart/drain, client-visible returned canceled assignment ids, and enforced modify leases, OpenAI Agents Python and Go agentsandbox local/cloud provider
