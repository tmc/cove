...
> [!NOTE]
> In the current gateway, `POST /v1/vms` is stubbed: it creates the operation record to exercise the long-running-operation plumbing, then immediately transitions the operation to `failed` with code `not_implemented`. Use `cove install` from the CLI to provision VMs until this lands. Clients can still use the shape below for retry and polling logic.
