...
- The public Go `agentsandbox` package and OpenAI Agents Python adapter now
  expose hosted audit-feed listing and audit-chain verification helpers for
  `/v1/audit` and `/v1/audit/verify`.
- The public Go `agentsandbox` package and OpenAI Agents Python adapter now
  expose hosted assignment cancellation and retry/replan controls, matching the
  audited `/v1/assignments/{id}/cancel` and `/v1/assignments/{id}/retry`
  controller operations.
- The public Go `agentsandbox` package and OpenAI Agents Python adapter now
  expose hosted worker lifecycle controls for cordon, uncordon, quarantine,
  unquarantine, evacuation plan/apply, drain, and decommission.
- The public Go `agentsandbox` package and OpenAI Agents Python adapter now
  expose hosted worker and assignment inventory with filters and pagination,
  giving agent clients the same controller visibility as `/v1/workers` and
  `/v1/assignments`.
- The public Go `agentsandbox` package and OpenAI Agents Python adapter now
...
- Image GC, lifecycle-policy, storage budget, and storage prune fan-out now
  ...
  without creating assignments, audit events, or retained run history.
- `POST /v1/images/prepare` now accepts `dry_run:true`, returning the same
  ...
  creating assignments, audit events, or retained preparation history.
- The public Go `agentsandbox` package and OpenAI Agents Python adapter now
...
- The public Go `agentsandbox` package and OpenAI Agents Python adapter now
  expose retained hosted placement-plan history with list/get helpers for
  `/v1/placements/plans` and `/v1/placements/plans/{id}`.
...
- `cove-fleetd` now exposes `GET /v1/assignments/{id}/events` for
  ...
  `assignment_id`.
- `cove-fleetd` now persists assignment worker reports and exposes
  `GET /v1/assignments/{id}/reports` with status, worker, offset, and limit
  filters.
...
- CI migration surface: landing-page draft, five-step quickstart, migration
  walkthrough, operator checklist, migration doctor VZScript, and May 2026
  benchmark report. Public install and registry language remain gated until
  release, privacy, trademark, and tap availability checks clear.
...
- Packer plugin shim (sunset; see `Non-goals`).
