...
volatile u64 entry_trap[16], entry_saved[16];
volatile u64 trace_meta[4], trace_pcs[128], supplied_counter;
...
static EFI_STATUS trace_entry(EFI_HANDLE child) {
    ...
    for (u64 i=0;i<nsec;i++) {
        ...
        if ((le32(sec+36)&0x20000000) && vs>=4 && rva>=va && rva-va<=vs-4) executable=1;
    }
    if (!executable) return EFI_UNSUPPORTED;
    if (rva > size-4 || (rva&3)) return EFI_UNSUPPORTED;
    u32 *entry=(u32*)(base+rva), original=*entry;
    if ((original&0xffffffe0)!=0xd53b9d00) return EFI_UNSUPPORTED;
    ...
    entry_saved[7]=(u64)entry; entry_saved[9]=original; entry_saved[10]=0; entry_saved[12]=(u64)base; entry_saved[13]=(u64)base+size-4;
    ...
    emit("original MDSCR "); emit_u64(entry_saved[8],1); emit("\n");
    ...
    emit("supplied CNTVCT "); emit_u64(supplied_counter,1); emit("\n");
    emit("trace stop reason "); emit_u64(trace_meta[1],0); emit(" (1=limit 2=before-system-instruction 3=synchronous-exception 4=image-boundary)\n");
    emit("steps "); emit_u64(trace_meta[0],0); emit(" next instruction "); emit_u64(trace_meta[2],1); emit("\n");
    for (u64 i=0;i<4;i++) { emit("x"); emit_u64(i+9,0); emit(" "); emit_u64(entry_trap[i+8],1); emit("\n"); }
    u64 first=trace_meta[0]>128 ? trace_meta[0]-128 : 0;
    for (u64 i=first;i<trace_meta[0];i++) { emit("pc["); emit_u64(i,0); emit("] "); emit_u64(trace_pcs[i&127],1); emit("\n"); }
    emit(result==1 && trace_meta[1] ? "BOUNDED TRACE RECOVERED\n" : "BOUNDED TRACE INCONCLUSIVE\n");
    emit("one-shot abandoned StartImage; this guest must be discarded\n"); flush_report();
    return EFI_ABORTED;
}
...
static EFI_STATUS chainload(EFI_HANDLE image, LOADED_IMAGE *li){
    ...
    emit("trace_entry returned "); emit_u64(ss, 1); emit(" (StartImage deliberately abandoned on breakpoint)\n");
    return ss;
}
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
...
#ifdef NO_SHIM
    emit("cove guest EFI entry breakpoint experiment\n");
    ...
    emit("NOTE: one natural PMCCNTR emulated; then short stepping with I/F masked; no code patch.\n");
