...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  reconciliation, and worker execution boundary with least-loaded and
  image-affinity placement; orchard still owns richer worker lifecycle and
...
expired-lease reconciliation, and worker-side `cove` assignment execution plus
