...
16. [v0.4 Anthropic adapter](archive/022-v04-anthropic-adapter.md) — v0.4 — Anthropic computer-use adapter mirroring the OpenAI Agents SDK adapter shape. Slice 1 SDK survey (Anthropic has no `ComputerTool` analogue; adapter drives the Messages API agent loop directly). Slice 2 `cove-claude-sandbox` Python package.
...
20. [Ephemeral self-hosted runners](026-ephemeral-self-hosted-runners.md) — shipped — disposable VM runner flow built around fork-from images and guest-agent command execution.
