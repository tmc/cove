...
6. **Image-aware, drainable fleet placement.** cove is not orchard's controller,
   ...
   placement-plan, warm-pool, and sandbox requests can require capabilities
...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  boundary with required-capability matching for assignments, warm pools, and
  sandboxes, least-loaded, image-affinity, and slot-capped bin-pack placement
