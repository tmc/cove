...
from design `021`. Ships the slices that
...
2. Each job inherits residue from the previous job (per
   `015`: 0 pass / 3 fail on warm-guest
...
- JIT-bytes window-of-disclosure: `WriteFile(/tmp/jit-config, bytes,
  ...
  / `021` §secrets) so the bytes never
...
proven; tracked in `021` Slice 2 layer.
...
GitHub Actions. Combined with `015`'s
...
- `015` — load-bearing for the
  "fork/restore is the only isolation primitive" claim.
- `021` — v0.4 CI executors. This
