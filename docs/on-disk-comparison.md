...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  cleanup, and per-pool lifecycle status counts, hosted-style sandbox create/list/get/delete/start/restart/stop/wait/lease/exec/control/metering handles
  ...
  drain for hosted sandbox maintenance and a reconciled operations summary;
  orchard still owns SAML identity binding and broader production controller
  operations.
