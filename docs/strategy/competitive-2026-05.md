...
1. **Canonical agent command polish.** `cove agent-sandbox run` is the one
   operator-facing path; next polish is provider-credential failure text,
   replay export recipes, and real private provider-run validation.
