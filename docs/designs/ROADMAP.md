...
| Slice 5 — stateful fleet controller (the MIT/paid boundary) | maybe | started | L | Slice 4 | [046](046-fleet-control-plane.md) §topology | Initial `cove-fleetd` host-inventory store, durable assignment leases, `least-loaded`/`image-affinity` placement, stale-worker/expired-lease reconciliation, controller-side worker cordon/uncordon, fleet image preparation, fleet image-GC push, lifecycle-policy push, storage budget/prune push, fork warm-pool quota reconciliation, agent-ready slot claim, persisted audit feed, and `coved -fleet-url` worker dial-out have landed: register, heartbeat, await-assignment, active `cove` assignment renewal, execution, and report-status are wired. Remaining: RBAC/SSO governance APIs. Controller is PAID; worker mode stays MIT. |
