...
| Network policy depth | Baseline policies and audit/log surfaces shipped, and `cove network audit <run-id-prefix>` now resolves run bundles into a summary with VM/image/status, policy, enforcement, allowlists, limitations, and decision counts while preserving `--raw` and `--json` modes. | Tart Softnet-style allow/block policy remains deeper. | Keep current enforcement limits explicit; consider DNS/egress allowlist policy only if a customer workload demands it. |
