...
| **Placement** | `fleet run --policy=least-loaded|image-affinity`; image-affinity can pre-stage a local image to the selected host; `fleet run -manifest-bundle <dir>` verifies an offline manifest bundle and requires remote/local image provenance to match the selected manifest digest before treating a mutable `-fork-from` ref as warm or safe to stage; `fleet run --all` fans out concurrently to every non-cordoned host and pre-stages `-fork-from` only to cold targets, with the same manifest-bundle exactness check when supplied; cordon/uncordon skips hosts for placement; short local placement leases count as pending load; `fleet health` reports remote reachability/version; repeated SSH operations reuse OpenSSH ControlMaster sockets; initial `cove-fleetd` control-plane boundary now accepts `coved -fleet-url` worker register/heartbeat/report dial-ins, auto-detects standard worker capabilities such as Darwin `ram-overlay`, DiskImages2 `asif`, and root live `apfs-quota`, tracks worker capabilities, image refs, and source manifest digests, persists priority-ordered controller assignment leases with queue-deadline expiry, per-assignment run timeouts, and bounded automatic retry, verifies offline `manifest_bundle` directories for assignment/placement-plan/warm-pool/sandbox/image-preparation admission, places assignments, warm pools, sandboxes, and maintenance fan-out with required worker capabilities, least-loaded, digest-exact image-affinity when manifest metadata is present, or bin-pack policy plus anti-affinity hints, persists retained top-k placement plan history with feasible candidates plus skipped-worker reasons for status/label/capability/capacity/image mismatch and list/get filters, reconciles stale workers and expired leases, exposes non-mutating reconciliation impact plans plus audited direct assignment creation plus worker-observed command/control cancellation and retry/replan, supports filterable/paginated worker and assignment inventory, worker-scoped hosted sandbox inventory, worker- and assignment-scoped event history, worker/assignment report history, and worker- and assignment-scoped usage metering, supports controller-side worker cordon/uncordon/quarantine/evacuate/drain/decommission with forced pending-work cancellation and operations-summary rollups, queues fleet image-preparation pulls from selected digest refs and force-refreshes stale mutable refs when a required manifest digest is provided, persists capability-filtered image-preparation, image-GC, lifecycle-policy, and storage budget/prune histories with list/get filters plus an aggregate `/v1/operations/runs` timeline, reconciles fork warm-pool quotas into `cove run -fork-from` assignments with optional image digest requirements, probes warm slots with `cove shell` before marking them ready, claims ready warm slots into same-worker guest exec assignments, stops claimed warm VMs after the guest command returns, downsizes warm pools by canceling pending surplus slots or queueing same-worker stop assignments for started surplus slots, exposes named warm-pool get/delete lifecycle operations with graceful claimed-slot deferral, per-pool lifecycle status counts, and warm-pool lifecycle event history, exposes hosted-style `/v1/sandboxes` create/list/filter/page/get/delete/start/restart/stop/wait/lease/exec/control/events/reports/metering handles with per-namespace `max_active_sandboxes` admission caps with active-sandbox diagnostics, queue priority, queue-deadline expiry, per-run timeout policy, queue-age/deadline diagnostics plus failed-admission placement plans, and bounded automatic retry over fork-run assignments with status-level attempt/retry-at/remaining diagnostics plus filterable retry state, ready probes, optional image digest requirements, terminal and target-status waits, stop cleanup surfaced on status/list/wait responses with filterable cleanup and lease state, retained-VM starts, restarts, status-visible and filterable open exec/control assignments plus sandbox-scoped cancellation during stop/delete/restart/drain with client-visible returned canceled ids, worker-enforced per-call exec/control timeouts, enforced TTL-based exclusive modify leases, same-worker shell exec, same-worker screenshot/key/text/mouse control proxying, and per-resource active-interval metering records, persists filterable/paginated hash-chained controller audit events with global verification plus worker and assignment filters, binds service-account bearer tokens to audit actors, scopes assignment/warm-pool/sandbox/service-account/audit/metering resources by service-account namespace, enforces `viewer`/`operator`/`admin` service-account roles, ships OpenAI Agents Python `SandboxRunConfig` and public Go `agentsandbox` local/cloud clients with capability-constrained hosted lifecycle result helpers, controller-side hosted wait-ready, hosted placement dry-runs, hosted placement history helpers, hosted image-preparation dry-runs with skipped-worker diagnostics and controls, hosted operations dashboard summaries, hosted reconcile plan/apply helpers, hosted assignment submission, hosted worker/assignment inventory, hosted scoped worker/assignment observability and fleet-level sandbox metering, hosted worker lifecycle controls, hosted assignment cancel/retry controls, hosted audit list/verify helpers, hosted service-account and OIDC/SAML identity/metadata helpers, hosted maintenance dry-runs, controls, skip diagnostics, and run history, hosted warm-pool lifecycle controls, paginated inventory, sandbox event/report history, fleet-level sandbox metering, and GUI event support, renews active `cove` assignments, and executes leased `cove` assignments asynchronously on workers (`cmd/cove/fleet_run.go`, `cmd/cove-fleetd`, `cmd/coved`, `internal/fleetcontrol`, `internal/coved`, `agentsandbox`, `adapters/openai-agents-python`) | none | none | full controller/scheduler model |
...
6. **Image-aware, drainable fleet placement.** cove is not orchard's controller,
   ...
   `ram-overlay`, DiskImages2-backed `asif`, and root live `apfs-quota`
   plus image source manifest digests, and assignment,
   ...
   before hosted work lands. Hosted assignment, sandbox, warm-pool, and
   image-preparation inventory can filter by manifest digest, digest ref,
   platform, image ref, and required capability, so operators can find exactly
   which queued or leased work, live handles, ready capacity, and pre-staging
   runs depend on an immutable image or RAM-overlay-class worker capability.
...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  initial `cove-fleetd`/`coved -fleet-url` host-inventory with auto-detected
  RAM-overlay, ASIF, and live-quota worker capabilities, image provenance, priority-ordered assignment leases with queue-deadline expiry, per-assignment run timeouts, and bounded automatic retry,
  reconciliation, worker cordon, digest-aware fleet image preparation with dry-run skip diagnostics and retained history, fleet image-GC push with dry-run planning and retained history,
  lifecycle-policy push with dry-run planning and retained history, storage budget/prune push with dry-run planning and retained history, aggregate retained controller-run feed, and worker execution
  ...
  retained placement plan history with skip diagnostics and list/get filters, and first warm-pool quota replenishment plus
  ...
  with per-namespace admission caps with active-sandbox diagnostics, queue priority, queue-deadline expiry, per-run timeout policy, queue-age/deadline diagnostics plus failed-admission placement plans, bounded automatic retry with status-level attempt/retry-at/remaining diagnostics and filterable retry state, server-side target-status waits, stop cleanup surfaced on status/list/wait responses with filterable cleanup and lease state, status-visible and filterable open exec/control assignments plus sandbox-scoped cancellation during stop/delete/restart/drain, client-visible returned canceled assignment ids, worker-enforced per-call exec/control timeouts, and enforced modify leases
  over fork-run assignments, OpenAI Agents Python and Go agentsandbox local/cloud
  provider switches with capability-constrained hosted lifecycle result helpers, placement
  dry-runs, placement history helpers, image-preparation dry-runs with skipped-worker diagnostics and controls, operations dashboard summaries, reconcile plan/apply helpers, assignment submission, worker/assignment inventory, scoped worker/assignment observability and fleet-level sandbox metering, worker lifecycle controls, assignment cancel/retry controls, audit list/verify helpers, service-account and OIDC/SAML identity/metadata helpers, maintenance dry-runs, controls, skip diagnostics, and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, fleet-level sandbox metering, and GUI events, plus persisted
  ...
  assignment creation plus worker-observed command/control cancellation, lifecycle-triggered hosted exec/control cancellation, and retry/replan, filterable/paginated worker and assignment inventory, worker-scoped hosted sandbox inventory, worker- and assignment-scoped event history, worker/assignment report history, worker- and assignment-scoped usage metering, non-mutating reconcile impact plans, and a reconciled operations summary; orchard still owns broader production
...
handles over fork-run assignments with per-namespace admission caps with active-sandbox diagnostics, queue priority, queue-deadline expiry, per-run timeout policy, queue-age/deadline diagnostics plus failed-admission placement plans, bounded automatic retry with status-level attempt/retry-at/remaining diagnostics and filterable retry state, server-side target-status waits, stop cleanup surfaced on status/list/wait responses with filterable cleanup and lease state, status-visible and filterable open exec/control assignments plus sandbox-scoped cancellation during stop/delete/restart/drain, client-visible returned canceled assignment ids, worker-enforced per-call exec/control timeouts, and enforced modify leases, OpenAI Agents Python and Go agentsandbox local/cloud provider
switches with capability-constrained hosted lifecycle result helpers, placement dry-runs, placement history helpers, image-preparation dry-runs with skipped-worker diagnostics and controls, operations dashboard summaries, reconcile plan/apply helpers, assignment submission, worker/assignment inventory, scoped worker/assignment observability and fleet-level sandbox metering, worker lifecycle controls, assignment cancel/retry controls, audit list/verify helpers, service-account and OIDC/SAML identity/metadata helpers, maintenance dry-runs, controls, skip diagnostics, and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, fleet-level sandbox metering, and GUI events, persisted per-resource sandbox usage
