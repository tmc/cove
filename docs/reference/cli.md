...
index resolution for later network-free `--manifest <path>` validation. Add
`--index-out <path>` with `--fetch-manifest` to write the exact top-level
...
`summary.json`, `index.json`, `selected.json`, and every fetched child under
`manifests/<digest>.json`; the summary records the pull target,
selected/index digests, platform, format, disk size/format, base reuse, blob
audit, and per-child audit status for offline CI or fleet policy checks.
