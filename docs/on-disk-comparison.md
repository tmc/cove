...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  worker decommission with forced pending-work cancellation, non-mutating
  reconcile impact plans, and a reconciled operations summary; orchard still owns broader production
...
expired-lease reconciliation with non-mutating impact plans, controller-side worker cordon/quarantine/evacuate/drain/decommission
