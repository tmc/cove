...
| Network policy depth | Baseline policies and audit/log surfaces shipped, every run now emits a `network_policy` metric, `cove network audit <run-id-prefix>` resolves run bundles into a summary with VM/image/status, policy, enforcement, allowlists, limitations, and decision counts while preserving `--raw` and `--json` modes, and `--net egress:<domain,ip,cidr...>` records custom per-run egress intent. | Tart Softnet-style allow/block policy remains deeper. | Keep current enforcement limits explicit; only claim audit/intent for NAT-backed allowlists until a hooked datapath exists. |
