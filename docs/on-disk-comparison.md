...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  cleanup, hosted-style sandbox create/list/get/delete/start/restart/stop/wait/lease handles
  over fork-run assignments, plus a persisted hash-chained controller audit feed,
...
hosted-style sandbox create/list/get/delete/start/restart/stop/wait/lease handles over
fork-run assignments, plus a persisted hash-chained audit feed with service-account
actor binding, namespace filters, and basic service-account roles, but tart and
