...
39. [macOS GUI-agent benchmark harness (CUA-bench-style)](047-gui-agent-benchmark-harness.md) — roadmap input, NotebookLM-backed (`7666f3d3`) — a CUA-bench-style task benchmark for NATIVE macOS agentic tasks, the OS no existing benchmark covers (OSWorld/WAA/AndroidWorld/WebArena cover Ubuntu/Windows/Android/web; ScreenSpot has macOS pixels but no live env). Adopts OSWorld's getter/metric verifier split + RAM-overlay fork-per-task reset; getters classified by macOS privilege tier (none / FDA / Apple-Events+AX); phased internal-eval → leaderboard → reusable harness. Two-round NotebookLM pressure-test corrected baseline-reset claims, the getter-grant model, cfprefsd/WAL flush discipline, slice order, and the minimum-credible-launch bar.
