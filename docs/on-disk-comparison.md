...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  RAM-overlay, ASIF, and live-quota worker capabilities, image provenance, assignment-lease,
  ...
  with per-namespace admission caps and enforced modify leases
...
boundary is present with auto-detected host capability inventory for RAM-overlay, ASIF, and live quotas, assignment leases, stale-worker and
...
handles over fork-run assignments with per-namespace admission caps and enforced modify leases, OpenAI Agents Python and Go agentsandbox local/cloud provider
