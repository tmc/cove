...
- **orchard has a real control plane.** cove's fleet support is CLI placement,
  ...
  `cove-fleetd`/`coved -fleet-url` host-inventory plus assignment-lease
  boundary; orchard still owns reconciliation, worker lifecycle, and
...
boundary is present with host inventory and assignment leases, but tart and lume
