...
| Slice 8 — hosted sandbox-by-API + SDK provider abstraction + metering | maybe | started | L | Slice 7 | [046](046-fleet-control-plane.md) §hosted | Initial `/v1/sandboxes` create/list/get/delete/start/restart/stop/wait/lease scaffold has landed: create maps to an image-affinity fork-run assignment, `coved` probes readiness with `cove shell`, stop/delete cancel pending sandboxes or queue same-worker stop cleanup, cleanup completion marks the handle `stopped`, start requeues canceled or stopped handles, restart queues stop cleanup then requeues the retained VM, and lease acquire/renew/release gives SDK clients an exclusive modify lock. Remaining: SDK `provider=local\|cloud` and per-resource metering. The macOS-fork wedge Cua Cloud structurally lacks. PAID/hosted, brand-gated. |
