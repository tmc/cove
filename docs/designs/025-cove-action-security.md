---
title: cove-action security architecture
description: stays private through this design; cove-action implementation deferred to v0.4; image push/pull is private-only until the public flip).
...
**Roadmap**: v0.4 — must land before `021`
...
`021` Slice 1. cove-action orchestrates
...
The load-bearing input is `015`: warm
...
4. **Residue between sequential jobs on the same parent** — the
   exact failure mode `015` measured
...
4. **`-ephemeral` cannot be disabled.** There is no
   ...
   `015`. The fork-end-of-job destroy
...
|---|---|---|
| `vzscripts/github-runner` | developer machine convenience; one-off self-hosted runner against a side branch; you accept warm-guest residue | soft-reset shape — see `015`, not a privacy boundary |
...
- `015` — load-bearing. Soft reset
  failed isolation 0/3/3; fork/restore is required.
- `021` — cove-action GHA wrapper
