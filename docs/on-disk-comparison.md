...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  with per-namespace admission caps with active-sandbox diagnostics, queue priority, queue-deadline expiry, per-run timeout policy, queue-age/deadline diagnostics plus failed-admission placement plans, bounded automatic retry with status-level attempt/retry-at/remaining diagnostics, server-side target-status waits, stop cleanup surfaced on status/list/wait responses, sandbox-scoped exec/control cancellation during stop/delete/restart/drain, client-visible returned canceled assignment ids, worker-enforced per-call exec/control timeouts, and enforced modify leases
