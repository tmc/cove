...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  agent-ready slot claim into guest `Exec` with stop and downsize cleanup;
  orchard still owns richer controller operations.
...
claim into same-worker guest `Exec` with stop and downsize cleanup, but tart
