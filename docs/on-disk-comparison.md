...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  cleanup, hosted-style sandbox create/list/get/delete/start/restart/stop/wait/lease/exec/control/metering handles
  ...
  RS256 OIDC bearer bindings with issuer discovery/JWKS refresh, plus worker
  drain for hosted sandbox maintenance; orchard still owns SAML identity binding
  and broader controller operations.
...
preparation, hosted sandbox worker drain, fleet image-GC push, lifecycle-policy push, storage budget/prune
...
hosted-style sandbox create/list/get/delete/start/restart/stop/wait/lease/exec/control/metering
