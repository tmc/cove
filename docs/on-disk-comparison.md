...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  reconciliation, worker cordon, digest-aware fleet image preparation with retained history, fleet image-GC push,
...
operations summary, manifest-bundle-backed digest-aware fleet image preparation with retained history, fleet image-GC push, lifecycle-policy push, storage budget/prune
