...
`main` = released. `next` = staging for the next tag.

- **`main`** = ship-ready. Tags cut from `main`. Only docs and merges
  from `next` land here outside of emergency fixes.
- **`next`** = parked work that didn't make the current release cut,
  plus the staging ground for the next release. Deferred blockers and
  half-baked experiments live here.
...
plus smoke-test history. Push it on the v0.1 release (see publish
checklist §3) and treat it as the staging trunk going forward; 0.2
feature branches all merge into it.

If `next` ever turns into a graveyard with no near-term release
candidate, collapse back to single-trunk and cut releases from `main`
directly.
