...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  placement plus anti-affinity hints, retained placement plans, and first
  warm-pool quota replenishment plus running-slot claim into guest `Exec`;
...
retained placement plans, warm-pool quota replenishment, and running-slot claim
into same-worker guest `Exec`, but tart and lume still lead in ecosystem
