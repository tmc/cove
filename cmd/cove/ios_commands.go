...
func runIOSCommand(env commandEnv, _ string, args []string) int {
	if len(args) > 0 && args[0] == "restore" {
		return runIOSRestore(env, args[1:])
	}
	...
	if len(args) == 1 && (args[0] == "help" || args[0] == "-h" || args[0] == "--help") {
		fmt.Fprintln(env.Stdout, "usage: cove ios preflight | restore probe|recovery-probe [flags] | devices [-libusb PATH] | new [-cpu N] [-memory GB] [-disk GB] NAME | setup source|patcher [flags] | firmware prepare|patch [flags] OUTPUT | config [flags] NAME | run [flags] NAME")
		return 0
	}
	if len(args) != 1 || args[0] != "preflight" {
		return commandUsageError(env, fmt.Errorf("usage: cove ios preflight | restore probe|recovery-probe [flags] | devices [-libusb PATH] | new [-cpu N] [-memory GB] [-disk GB] NAME | setup source|patcher [flags] | firmware prepare|patch [flags] OUTPUT | config [flags] NAME | run [flags] NAME"))
	}
