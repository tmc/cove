...
| Slice 5 — stateful fleet controller (the MIT/paid boundary) | maybe | started | L | Slice 4 | [046](046-fleet-control-plane.md) §topology | Initial `cove-fleetd` host-inventory store, durable assignment leases, `least-loaded`/`image-affinity` placement, stale-worker/expired-lease reconciliation, controller-side worker cordon/uncordon, fleet image preparation, and `coved -fleet-url` worker dial-out have landed: register, heartbeat, await-assignment, active `cove` assignment renewal, execution, and report-status are wired. Remaining: fork warm pools. Controller is PAID; worker mode stays MIT. |
...
| Slice 7 — top-k bin-pack scheduler + base-image affinity + fork-warm-pool | maybe | started | L | Slice 5 | [046](046-fleet-control-plane.md) §scheduler | Base-image affinity placement, slot-capped `bin-pack` placement, and the first controller image-preparation endpoint have landed; remaining work is top-k retained node scores, anti-affinity terms, and fork warm pools. Image-locality is cove's edge over Orchard/Daytona. PAID. |
