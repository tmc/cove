...
volatile u64 trace_meta[4], trace_pcs[128];
...
static void sync_code(void *p) { __asm__ volatile("dc cvau, %0\n dsb ish\n ic ivau, %0\n dsb ish\n isb" :: "r"(p) : "memory"); }
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
    ...
    emit("NOTE: this bounded trace is instrumented execution, not an original-stall sample.\n");
