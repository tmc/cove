...
| Network policy depth | Baseline policies and audit/log surfaces shipped, every run now emits a `network_policy` metric, `cove runs show` and GitHub summaries fold that metric into network summaries, `cove network audit <run-id-prefix>` resolves run bundles into a summary with VM/image/status, policy, enforcement, allowlists, limitations, and decision counts while preserving `--raw` and `--json` modes, and `--net egress:<domain,ip,cidr...>` records custom per-run egress intent. | Tart Softnet-style allow/block policy remains deeper. | Keep current enforcement limits explicit; only claim audit/intent for NAT-backed allowlists until a hooked datapath exists. |
...
next non-gated gap to close is canonical agent command polish, followed by
in-VM user management. The larger public
...
| Image lifecycle | Local image store, build/list/rm/inspect/gc, disk-format provenance for raw/ASIF images, tar push/load, OCI push/pull via `oras-go`, Docker auth reuse, public-push refusal still active, and remote inspect now reports index/list resolution, selected platform, pull plan, cove base-chain audit, verification posture, and optional HEAD-only descriptor/blob audit for cove/Tart/Lume/image-store artifacts before download. | Lume has GHCR-style pull/push, chunked upload, reassemble, cache/storage config. | Tart has mature OCI push/pull to any registry, public image catalog, Packer templates, and ASIF defaults. | Old gap partly closed. Remaining gap is public signed/curated channel, templates, and full ASIF distribution polish. |
| Fork/ephemeral isolation | Strong lead: APFS clonefile fork, image-backed ephemeral forks, soft-reset rejected as isolation primitive, GHA action uses fresh fork per job, and `cove runs show`/GitHub summaries now expose fork provenance, source manifest digest, materialization mode, disk reuse, cleanup intent, verification, and limitations from `fork_created` metrics. | Clone exists; Cua Sandbox exposes ephemeral API, but Lume docs do not give hard VM fork isolation contract. | Clone/standby/VM isolation exist, but task isolation often CI/persistent-worker-shaped. | Cove leads on explicit hard-isolation story. |
...
- named policies: `offline`, `packages`, `host-services`, `lan`, `open`, plus
  custom `egress:<domain,ip,cidr...>` allowlist intent.
