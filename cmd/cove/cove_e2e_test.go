...
func TestNestedHelpDoesNotTreatHelpAsOperand(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "9p serve", args: []string{"9p", "serve", "-h"}, want: "Usage: cove 9p serve"},
		{name: "ctl status", args: []string{"ctl", "status", "-h"}, want: "Usage: cove ctl status"},
		{name: "ctl screenshot", args: []string{"ctl", "screenshot", "-h"}, want: "Usage: cove ctl screenshot"},
		{name: "ctl ocr", args: []string{"ctl", "ocr", "-h"}, want: "Usage: cove ctl ocr"},
		{name: "ctl iterm2 proxy", args: []string{"ctl", "iterm2-proxy", "-h"}, want: "Usage: cove ctl iterm2-proxy"},
		{name: "ctl disk resize", args: []string{"ctl", "disk", "resize", "-h"}, want: "Usage: cove ctl disk resize"},
		{name: "sip enable auto", args: []string{"sip", "enable-auto", "-h"}, want: "Usage: cove sip enable-auto"},
		{name: "sip disable auto", args: []string{"sip", "disable-auto", "-h"}, want: "Usage: cove sip disable-auto"},
		{name: "inject alias", args: []string{"inject", "-h"}, want: "Usage: cove inject"},
		{name: "inject agent alias", args: []string{"inject-agent", "-h"}, want: "Usage: cove inject-agent"},
		{name: "daemon ui", args: []string{"daemon", "ui", "-h"}, want: "Usage: cove daemon ui"},
		{name: "pins list", args: []string{"pins", "list", "-h"}, want: "Usage: cove pins list"},
		{name: "fleet vm ls", args: []string{"fleet", "vm", "ls", "-h"}, want: "Usage: cove fleet vm ls"},
		{name: "fleet ps", args: []string{"fleet", "ps", "-h"}, want: "Usage: cove fleet ps"},
