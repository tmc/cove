...
- **orchard has a real control plane.** cove's fleet support is CLI placement,
  ...
  execution boundary; orchard still owns reconciliation, worker lifecycle, and
...
`cove` assignment execution, but tart and lume still lead in ecosystem maturity,
and orchard still leads as a full fleet controller.
