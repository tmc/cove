...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  RAM-overlay, ASIF, and live-quota worker capabilities, image provenance, priority-ordered assignment leases with queue-deadline expiry,
  ...
  with per-namespace admission caps, queue priority, queue-deadline expiry, and enforced modify leases
...
boundary is present with auto-detected host capability inventory for RAM-overlay, ASIF, and live quotas, priority-ordered assignment leases with queue-deadline expiry, stale-worker and
...
handles over fork-run assignments with per-namespace admission caps, queue priority, queue-deadline expiry, and enforced modify leases, OpenAI Agents Python and Go agentsandbox local/cloud provider
