...
| Fork/ephemeral isolation | Strong lead: APFS clonefile fork, image-backed ephemeral forks, soft-reset rejected as isolation primitive, GHA action uses fresh fork per job, and `cove runs show`/GitHub summaries now expose fork provenance, materialization mode, disk reuse, cleanup intent, verification, and limitations from `fork_created` metrics. | Clone exists; Cua Sandbox exposes ephemeral API, but Lume docs do not give hard VM fork isolation contract. | Clone/standby/VM isolation exist, but task isolation often CI/persistent-worker-shaped. | Cove leads on explicit hard-isolation story. |
