...
| Slice 5 — stateful fleet controller (the MIT/paid boundary) | maybe | started | L | Slice 4 | [046](046-fleet-control-plane.md) §topology | Initial `cove-fleetd` host-inventory store, durable assignment leases, `least-loaded`/`image-affinity` placement, stale-worker/expired-lease reconciliation, controller-side worker cordon/uncordon, fleet image preparation, fleet image-GC push, fork warm-pool quota reconciliation, agent-ready slot claim, and `coved -fleet-url` worker dial-out have landed: register, heartbeat, await-assignment, active `cove` assignment renewal, execution, and report-status are wired. Remaining: lifecycle policy push and governance APIs. Controller is PAID; worker mode stays MIT. |
| Slice 6 — fleet-wide policy & GC push | maybe | started | M | Slice 5 | [046](046-fleet-control-plane.md), [031](031-vm-lifecycle.md), [040](040-storage-budget.md) | Controller now pushes image-GC assignments down worker streams with skip reporting and image-ref refresh. Remaining: lifecycle policy push and storage-budget/GC policy fan-out. PAID. |
