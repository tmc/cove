...
static EFI_STATUS chainload(EFI_HANDLE image, LOADED_IMAGE *li){
    ...
    EFI_STATUS ss = BS->StartImage(child, 0, 0);
    emit("StartImage returned "); emit_u64(ss, 1); emit(" (bootmgfw exited)\n");
    return ss;
}
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
...
#ifdef NO_SHIM
    emit("cove Windows-on-VZ CHAINLOAD control (experiment #4, NO_SHIM)\n");
    ...
    emit("NOTE: advancing disk reads prove PROGRESS only, not sole-blocker,\n");
    emit("      not shippability. Direct linear-fb writes bypass Blt, so NO\n");
    emit("      post-ExitBootServices screen update is expected.\n");
    flush_report();
