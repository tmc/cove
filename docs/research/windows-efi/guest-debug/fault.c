...
static EFI_STATUS trace_entry(EFI_HANDLE child) {
    ...
    if (original != 0xd53b9d09) { emit("expected MRS x9, PMCCNTR_EL0; refusing\n"); return EFI_UNSUPPORTED; }
    ...
    emit(result==1 && entry_trap[0]==(u64)entry && entry_trap[1]==0x02000000 ? "EXACT PMCCNTR FAULT RECOVERED\n" : "FAULT CONTROL NO MATCH\n");
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
...
#ifdef NO_SHIM
    emit("cove unchanged-image PMCCNTR fault observation\n");
    ...
    emit("NOTE: no code patch, no stepping; custom synchronous exception vector only.\n");
