...
volatile u64 entry_trap[8], entry_saved[12];
...
static EFI_STATUS trace_entry(EFI_HANDLE child) {
    ...
    emit("\nentry RVA "); emit_u64(rva,1); emit(" expected ELR "); emit_u64((u64)entry,1);
    emit("\noriginal instruction "); emit_u64(original,1); emit("\n"); flush_report();
    entry_saved[7]=(u64)entry; entry_saved[9]=original; entry_saved[10]=0;
    ...
    *entry=original; sync_code(entry);
    ...
    emit(result==1 && entry_trap[0]==(u64)entry+4 && (entry_trap[1]>>26)==0x33 ? "ONE INSTRUCTION STEP PASS\n" : "ONE INSTRUCTION STEP INCONCLUSIVE\n");
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
    ...
    emit("NOTE: this one-shot test observes entry only, not the original stall PC.\n");
