...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  binding, and namespace-scoped controller resources; orchard still owns richer
  RBAC/SSO and controller operations.
...
plus a persisted audit feed with service-account actor binding and namespace
filters, but tart and lume still lead in ecosystem maturity, and orchard still
