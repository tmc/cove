...
- **Audit log:** single-host `~/.vz/metrics.jsonl` stays MIT; the fleet-wide
  ...
  deterministic `prev_hash`/`hash` chain, `GET /v1/audit` supports
  namespace/actor/action/target filters with offset/limit page metadata,
  `GET /v1/audit/verify` verifies the global chain for unscoped viewers, and
  `/v1/oidc-bindings` now maps RS256-verified OIDC JWTs into namespace-scoped
  viewer/operator/admin identities with issuer discovery, JWKS caching, and
  key-miss refresh. `/v1/saml-bindings` now stores and audits fail-closed SAML
  IdP configuration records with validated X.509 signing certificates; the
  remaining work is verified SAML assertion authentication.
...
- **API shape**, modeled on the fly.io Machines API
  ...
  /v1/sandboxes/{id}/events` for sandbox-scoped audit history, `GET
  /v1/sandboxes/{id}/reports` for sandbox-scoped worker report history, and
  `GET /v1/sandboxes/{id}/metering` and `GET /v1/metering/sandboxes` for
  scoped usage records. `GET /v1/sandboxes` now filters by status, worker,
  image, namespace, offset, and limit, and returns `next_offset`, so hosted
  clients can page through only the handles they need; the Go and Python SDKs
  expose the same filters and page metadata. The OpenAI Agents Python adapter now has a `provider="cloud"`
  ...
  create/list/get/wait/lease/restart/exec/control/events/reports/metering/delete
  and screenshot/key/text/mouse flows; both SDKs carry held lease holders into
...
| 9 | Fleet RBAC / SSO / audit | SAML/OIDC, namespaces, fleet audit log; initial persisted filterable/paginated audit feed, service-account actor binding, namespace filters, service-account roles, audit-chain verification, RS256 OIDC bearer bindings with discovery/JWKS refresh, and fail-closed SAML IdP binding records have started | PAID | Slice 5 | Enterprise-governance monetization axis (Teleport line); the durable revenue, not the scheduler. |
