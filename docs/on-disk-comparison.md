...
6. **Image-aware, drainable fleet placement.** cove is not orchard's controller,
   ...
   controller path, `coved` heartbeats now report image source manifest digests,
...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  initial `cove-fleetd`/`coved -fleet-url` host-inventory with image
  provenance, assignment-lease,
  reconciliation, worker cordon, digest-aware fleet image preparation, fleet image-GC push,
  ...
  assignment cancellation and retry/replan, filterable/paginated worker and assignment inventory, worker-scoped hosted sandbox inventory, worker- and assignment-scoped event history, worker/assignment report history, worker-scoped usage metering, non-mutating reconcile impact plans, and a reconciled operations summary; orchard still owns broader production
...
expired-lease reconciliation with non-mutating impact plans, audited direct assignment cancellation and retry/replan, filterable/paginated worker and assignment inventory, worker-scoped hosted sandbox inventory, worker- and assignment-scoped event history, worker/assignment report history, and worker-scoped usage metering, controller-side worker cordon/quarantine/evacuate/drain/decommission
