...
cove image inspect -remote <registry/ref:tag|digest>... [-json] [-platform os/arch[/variant]] [-all-platforms] [-verify-blobs] [-manifest-out <path>]
...
cove image inspect -remote registry.example/team/macos-runner:14.5 registry.example/team/linux-runner:latest -json
...
| Command | Description |
|---------|-------------|
| `gc [-dry-run]` | Garbage collect unreachable store blobs. GC takes an exclusive store lock, keeps blobs modified within the last hour, and treats VM `disk.provenance`, local image `source_manifest_digest`, and build-cache metadata as roots. `-dry-run` prints candidate deletion totals without deleting blobs. |
...
index resolution for later network-free `--manifest <path>` validation. When a
manifest is available, text and JSON dry-runs include a digest-pinned selected
ref; index/list resolutions also include the top-level index digest ref.
...
| `--format <fmt>` | cove | Output format: cove, lume, or tart |
