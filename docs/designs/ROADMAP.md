...
| GitHub Actions private executor surface | done | local image store + run metrics | [021](archive/021-v04-ci-executors-tracks.md), [030](030-gha-executor-slice-2.md) | Ships the private GHA wrapper, action metrics, preflight commands, and cache-image reuse. Implementation: `0985377` (cove-action runner), `8bd473e` (action metrics emission), `82a0ac5` (metrics path), `7fafe40` (preflight commands), `9e6253a` (Slice 2 local image cache restore); accompanying executor docs at `19804c7`. |
...
| Fleet Slice 1 remote routing | done | SSH access to trusted Macs | [034](034-fleet-slice-1.md) | Adds fleet host config, SSH tunnel helpers, routeable remote commands, and docs. Design doc at `622b571`; implementation `695ae2e` (remote config + SSH tunnel helpers), `9f993a5` (CLI + remote command routing), `366bfac` (quickstart docs). |
| Fleet Slice 2 aggregation | done | Fleet Slice 1 | [034](034-fleet-slice-1.md) | Adds parallel remote query helpers and aggregate VM/image/ps lists. Design doc at `afba1a5`; implementation `0b4776f` (parallel query helper), `e59348d` (aggregate VM + image lists), `6e91044` (aggregate `ps`). |
...
| APFS clonefile-aware storage census | should | allocated bytes shipped (`832c750`); clone de-dup deferred | S | design 040 phases 0-5 | [040](040-storage-budget.md) §Open questions | Storage census now counts allocated bytes, which fixes sparse-file overstatement for budget reporting. One-pass clone reference counting remains deferred until user-visible clone double-counting is proven. Deferred-boundary audit at `4a3029d`. |
| Guest → host artifact copy-out (`cove runs export --include-guest`, cove-action `artifacts`) | should | shipped (`9b88e8e` plus cove-action convention) | M | guest agent + runs bundle | [Cirrus migration readiness 2026-05-08](../strategy/cirrus-migration-readiness-2026-05-08.md) §Partial | `cove runs export --include-guest` and cove-action `artifacts` copy guest artifacts into the run bundle so CI scripts do not need trailing hand-written copy commands. |
...
| Test coverage gap fill — control + storage internals | should | saturated; local gates green (`cd14f9e`); main CI green on cove-hermes-v10 | M | none | R57-R111 test-gap dispatches | Coverage sweeps filled small/medium branches across storage, controlclient, controlserver, windows, runs, sckit, ociimage, and related daemon surfaces. Final cleanup at `cd14f9e` makes `go build ./...`, `go vet ./...`, and `go test ./...` pass on local main; `1c31744`/`0d5b7af` move the private PR workflow onto the cove runner with setup-go cache disabled, and R132 tightens local image/build-scratch store permissions at `4262a85`. Further coverage work should start from a fresh low-watermark audit rather than this v0.6 sweep row. |
| Deadcode + staticcheck SA4006 sweep — round 5 | should | shipped (`78be1fd`, 2026-05-10) | S | none | R54-R94 sweeps | U1000/SA4006 cadence shipped through R54 (`1bdd1e8`), R56 (`260e80c`, `05d0ee3`, `8d53d70`, `59be48a`), R57 (`aa1a80e`), R59 (`e3f135a`), R83 (`2decbf9`), and R94 (`78be1fd`). Verified `staticcheck -checks=U1000,SA4006 ./...` clean on 2026-05-10. |
| Capture-latency observability surface | maybe | shipped (`ecbf3bb`) | M | Slice 3 perf wiring | [042](042-capture-latency-observability.md) | Design 042 names the `capture_latency` run event, low-cardinality `coved_capture_*` Prometheus series, and insertion points through `screenshots.go`, run bundles, and `internal/coved/prometheus.go`. R106 added daemon-side latency aggregation; R111 wires production `captureDisplayImage` samples into run metrics. |
...
| SCKit Slice 2 perf numbers — p50/p95 under TCC + GUI VM | must | blocked: needs operator hardware | Screen Recording TCC grant + representative GUI VM | [041](041-screencapturekit-migration.md) §Slice 2 perf gate, [041 preflight](041-slice4-preflight.md) | Collect the latency evidence required before Slice 4 flips the default. The required artifact is a TCC-granted run of `cove doctor sckit-spike -n 30 -threshold 50ms -title-prefix cove` plus live `internal/sckit` tests against a visible cove GUI VM; do not infer from the spike shipping alone. |
| Cirrus shutdown 2026-06-01 closeout | should | blocked until 2026-06-01 | v0.6 Cirrus secrets surface + real post-shutdown docs pass | [readiness audit](../strategy/cirrus-migration-readiness-2026-05-08.md), [migration checklist](../migrations/from-cirrus-checklist.md) | After 2026-06-01, verify migration docs, benchmark notes, and landing pages against the actual shutdown state, then remove rollback language that only applied while Cirrus still existed. Do not pre-close this before the date passes. |
| macOS 15 host audit | should | blocked: needs operator host evidence | v0.6 macOS 14 floor + TCC-granted SCKit bench host | [macOS floor audit](../strategy/macos-floor-2026-05-08.md), [041 preflight](041-slice4-preflight.md) | Current local docs establish the v0.6 macOS 14 floor and do not justify a v0.7 floor change. Before deciding whether v0.7 remains macOS 14 or bumps to 15, record a macOS 15 host run of `cove doctor sckit-preauth`, live `internal/sckit` capture tests with a visible GUI VM, `cove doctor sckit-spike -n 30 -threshold 50ms -title-prefix cove`, and any Virtualization.framework behavior differences observed during that run. |
...
| Apple App Sandbox proof lane | should | initial proof boundary landed; packaging proof queued | signed build + packaging proof | [045](045-app-sandbox.md) | Explore whether cove can run with `com.apple.security.app-sandbox` without weakening the operator model. The opt-in harness now proves a raw ad-hoc sandboxed CLI can be signed but traps before `--version`, `help`, and `list`; `security status` and `doctor host` report host App Sandbox state separately from guest containment; host elevation now fails closed when App Sandbox is active; design 045 records supported package shapes and claim boundaries. Next work is `.app`/worker architecture proof, not a simple entitlement flip. |
...
| Item | Priority | Status | Size | Depends on | Source | Why |
|---|---|---|---|---|---|---|
| Slice 4 — maximize stateless SSH (pooling, concurrent multi-host run, parallel health probes) | done | shipped | M | 034 S1-3 | [046](046-fleet-control-plane.md) §slices | Exhausts the stateless design space (all 034-deferred) with no new trust surface; burst CI capacity. Stays MIT. |
| Slice 5 — stateful fleet controller (the MIT/paid boundary) | maybe | started | L | Slice 4 | [046](046-fleet-control-plane.md) §topology | Initial `cove-fleetd` host-inventory store, durable assignment leases, `least-loaded`/`image-affinity` placement, stale-worker/expired-lease reconciliation, controller-side worker cordon/uncordon/drain, fleet operations summary, fleet image preparation, fleet image-GC push, lifecycle-policy push, storage budget/prune push, fork warm-pool quota reconciliation, agent-ready slot claim, filterable/paginated persisted audit feed, service-account RBAC, RS256 OIDC bearer bindings with issuer discovery/JWKS refresh, fail-closed SAML IdP binding records, and `coved -fleet-url` worker dial-out have landed: register, heartbeat, await-assignment, active `cove` assignment renewal, execution, and report-status are wired. Remaining: verified SAML assertion authentication. Controller is PAID; worker mode stays MIT. |
| Slice 6 — fleet-wide policy & GC push | maybe | shipped | M | Slice 5 | [046](046-fleet-control-plane.md), [031](031-vm-lifecycle.md), [040](040-storage-budget.md) | Controller now pushes image-GC, VM lifecycle-policy, and storage budget/prune assignments down worker streams with skip reporting. PAID. |
| Slice 7 — top-k bin-pack scheduler + base-image affinity + fork-warm-pool | maybe | shipped | L | Slice 5 | [046](046-fleet-control-plane.md) §scheduler | Base-image affinity placement, slot-capped `bin-pack` placement, anti-affinity hints, the first controller image-preparation endpoint, retained top-k placement plans, durable warm-pool quota replenishment, agent-ready slot claim/Exec handoff, claimed-slot stop cleanup, warm-pool downsize cleanup, named pool get/delete lifecycle operations, and richer per-pool lifecycle status counts have landed. Image-locality is cove's edge over Orchard/Daytona. PAID. |
| Slice 8 — hosted sandbox-by-API + SDK provider abstraction + metering | maybe | started | L | Slice 7 | [046](046-fleet-control-plane.md) §hosted | Initial `/v1/sandboxes` create/list/get/delete/start/restart/stop/wait/lease/exec/control/events/reports/metering scaffold has landed: create maps to an image-affinity fork-run assignment, list supports namespace/status/worker/image/offset/limit filters plus page metadata across the REST API and SDKs, `coved` probes readiness with `cove shell`, stop/delete cancel pending sandboxes or queue same-worker stop cleanup, cleanup completion marks the handle `stopped`, start requeues canceled or stopped handles, restart queues stop cleanup then requeues the retained VM, lease acquire/renew/release gives SDK clients an enforced exclusive modify lock for start/restart/stop/delete/exec/control, exec queues same-worker `cove shell` work and returns worker reports, control proxies same-worker screenshot/key/text/mouse events through the VM control socket, events returns sandbox-scoped audit history, reports returns sandbox-scoped worker output history, append-only metering records active run intervals by VM, CPU, and memory-byte milliseconds, and the OpenAI Agents Python adapter plus public Go `agentsandbox` package now support local/cloud SDK flows for create/list/get/wait/lease/restart/exec/control/events/reports/metering/delete and hosted GUI events. Remaining: hosted polish and public API hardening. The macOS-fork wedge Cua Cloud structurally lacks. PAID/hosted, brand-gated. |
| Slice 9 — fleet RBAC / SSO / audit | maybe | started | L | Slice 5 | [046](046-fleet-control-plane.md) §security | Initial persisted controller audit feed, `GET /v1/audit` with namespace/actor/action/target/sandbox filters and offset/limit page metadata, deterministic `prev_hash`/`hash` audit chaining with `GET /v1/audit/verify`, sandbox-scoped event history, service-account records, bearer-token audit actor binding, namespace-scoped assignment/warm-pool/sandbox/service-account/audit filtering, `viewer`/`operator`/`admin` roles, admin-managed RS256 OIDC issuer/subject/audience bindings with issuer discovery/JWKS caching/key-miss refresh, and fail-closed SAML IdP binding records with validated X.509 signing certificates have landed. Remaining: verified SAML assertion authentication. This is the durable governance monetization axis (Teleport line). PAID. |

## Benchmark horizon — macOS GUI-agent benchmark (CUA-bench-style)

**Status: design input, not scheduled.** `cove agent-sandbox` runs computer-use
agents but only *runs* them; today's `agent-sandbox bench` measures latency and
error rate, not task completion. [047](047-gui-agent-benchmark-harness.md) is the
NotebookLM-backed design ( notebook `7666f3d3`, two-round adversarial
pressure-test) for a CUA-bench-style benchmark that *scores* agents against a
corpus of verifiable **native macOS** tasks — the OS no existing benchmark covers
(OSWorld is Ubuntu/Windows, WAA is Windows, AndroidWorld is Android, WebArena is
web-only; ScreenSpot has macOS pixels but no live resettable environment, and
running macOS off Apple hardware is legally blocked, so cove on Apple Silicon is
the answer). It adopts OSWorld's getter/metric verifier split and a RAM-overlay
fork-per-task reset (~130 ms, faster + more hermetic than every baseline's
reset), classifies getters by macOS privilege tier (none / FDA / Apple-Events+AX),
and phases internal-eval → leaderboard → reusable harness. Gated by the same
trademark/brand + public/private decisions before any *public* leaderboard.

| Item | Priority | Status | Size | Depends on | Source | Why |
|---|---|---|---|---|---|---|
| Slice 1 — task schema (parameterized) + verifier library (no VM) | maybe | design | M | 038 provider abstraction | [047](047-gui-agent-benchmark-harness.md) §9 | OSWorld-shape declarative task + pure-function metrics, unit-testable without a VM; foundational parameterization for anti-memorization. |
| Slice 2 — runner + fork-reset proof + crash/checkpoint-resume | maybe | design | M | Slice 1, 013 fork | [047](047-gui-agent-benchmark-harness.md) §9 | fork→setup→agent→flush→verify→score over `cove run -fork-from -ephemeral`; proves the RAM-overlay fork resets cfprefsd/TCC/Spotlight/SQLite between tasks. |
| Slice 3 — pre-granted base image + Tier-C getters (AX + AppleScript) | maybe | design | M | Slice 2, TCC doctor | [047](047-gui-agent-benchmark-harness.md) §5, §9 | AX-tree is the reliable synchronous GUI-state probe; must exist before the corpus or every task gets brittle OCR verifiers. FDA + AE/AX baked into the image. |
| Slice 3b — structured AX snapshot over the socket | maybe | design | M | Slice 3, [048](048-guest-ax-snapshot-over-socket.md) | [047](047-gui-agent-benchmark-harness.md) §5, §16 / [048](048-guest-ax-snapshot-over-socket.md) | Deepens Slice 3 AX-tree fidelity: the shipped getter emits a four-attribute XML tree via one-shot JXA; this surfaces an in-guest `AXSnapshot` RPC over the control socket (mirroring OCR-over-socket) carrying per-element **geometry, enabled/settable, available actions, and a stable element index** — the Codex `get_app_state` shape — reusing axmcp's `computeruse/appstate` semantics on the shared `apple/x/axuiautomation` dep. Raises `accessibility_match` precision and feeds Slice 4b trajectory grounding. XML dumper stays as the zero-dep fallback; resolves 047 §13 AX-getter open question. |
| Slice 4 — seed corpus v0 (10–20) + verifier self-check (internal-eval) | maybe | design | M | Slice 3 | [047](047-gui-agent-benchmark-harness.md) §9 | First cove-original macOS tasks + a known-good/no-op self-check that the verifier is correct; realizes Phase-1 dogfooding value. |
| Slice 5 — corpus growth ≥116–154 + multi-provider scoring + variance | maybe | design | L | Slice 4 | [047](047-gui-agent-benchmark-harness.md) §11 | Parity-floor scale across ≥3 providers + human baseline, pass@1 over ≥3 runs; the leaderboard-credibility floor. |
...
- **2026-05-30**: Added design [048](048-guest-ax-snapshot-over-socket.md) (guest-side structured AX snapshot over the control socket) and **Benchmark-horizon Slice 3b**, from a deep pass over [`github.com/tmc/axmcp`](https://github.com/tmc/axmcp). Resolves the [047](047-gui-agent-benchmark-harness.md) §13 AX-getter open question: the shipped getter emits a four-attribute XML AX tree via one-shot JXA; 048 specs an in-guest `AXSnapshot` RPC surfaced over the socket (mirroring OCR-over-socket) carrying per-element geometry, enabled/settable, actions, and a stable element index — the Codex `get_app_state` shape — reusing axmcp's `computeruse/appstate` semantics. **Rejected** proxying *host*-run axmcp: `apple/x/axuiautomation` is a local-OS binding (`AXUIElementCreateApplication` targets a same-OS PID), so the guest AX tree can only be read in-guest — the same host/guest boundary as OCR. Transport 2b (single `AXSnapshot` RPC on the existing agent) preferred over 2a (in-guest MCP). Version skew (axmcp go 1.26 + apple v0.6.9 vs cove go 1.25.5 + v0.6.11) means **vendor the snapshot builder, don't import axmcp**; the shared `apple/x/axuiautomation` dep makes the lift MCP-free. XML dumper stays as the zero-dep fallback. Docs-only; no code, no public surface (privacy gate).
- **2026-05-29**: Added the **Benchmark horizon** milestone and design [047](047-gui-agent-benchmark-harness.md) (NotebookLM-backed, notebook `7666f3d3`, two-round adversarial pressure-test). Roadmaps a CUA-bench-style benchmark that *scores* computer-use agents against a corpus of verifiable **native macOS** tasks — the macOS-native gap no existing benchmark fills (OSWorld/WAA/AndroidWorld/WebArena cover Ubuntu/Windows/Android/web; ScreenSpot has macOS pixels but no live env; macOS off Apple hardware is legally blocked). Adopts OSWorld's getter/metric verifier split + RAM-overlay fork-per-task reset; getters classified by macOS privilege tier (none / FDA / Apple-Events+AX); phased internal-eval → leaderboard (≥116–154 tasks, human-verified tier) → reusable harness. Today's `agent-sandbox bench` measures only latency/error-rate; this adds task-completion scoring. All cove-substrate claims verified against `origin/main`; benchmark-field claims primary-sourced + adversarially cross-checked (refuted a "WAA restarts VM per task" claim and an OSWorld-Verified contamination over-claim). All rows `maybe`/unscheduled; public leaderboard brand/privacy-gated.
...
- **2026-05-10** (R425): Fleet Slice 2 row separated design-doc SHA from implementation. `afba1a5` is "docs: design fleet slice 2 aggregation"; `0b4776f` is the parallel query helper; `e59348d` is aggregate VM + image lists; `6e91044` is aggregate `ps`.
- **2026-05-10** (R424): Fleet Slice 1 row separated design-doc SHA from implementation. `622b571` is "docs: design fleet slice 1"; `695ae2e` is the remote config + SSH tunnel helpers; `9f993a5` is CLI + remote command routing; `366bfac` is quickstart docs.
- **2026-05-10** (R422): Fresh VM login recovery cluster row separated bug-doc SHA from implementation. `b9c06ee` is "docs: record fresh vm login bug"; the four impl SHAs (`e512329`/`a1742f4`/`14e2bdb`/`ca4d824`) ship the login-prompt classifier, root requirement, noninteractive-auth refusal, and diskutil-output dump.
- **2026-05-10** (R107): Refreshed v0.6 candidate statuses against current `origin/main`: allocated-byte storage census (`832c750`), guest artifact export (`9b88e8e`), capture latency spec (`aee83c5`) plus daemon aggregation (`864fb59`), and deferred blocker audit (`4a3029d`).
- **2026-05-10** (R420): GitHub Actions private executor surface row separated docs SHA from implementation. `19804c7` is "docs: describe GitHub Actions executor"; the five impl SHAs (`0985377`/`8bd473e`/`82a0ac5`/`7fafe40`/`9e6253a`) ship the runner, metrics, preflight, and Slice 2 cache restore.
