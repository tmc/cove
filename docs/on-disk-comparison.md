...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  cleanup, plus a persisted controller audit feed and service-account actor
  binding; orchard still owns richer RBAC/SSO and controller operations.
...
plus a persisted audit feed with service-account actor binding, but tart and
