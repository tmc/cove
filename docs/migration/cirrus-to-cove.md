...
| GitHub Actions annotations from-guest | write `error`, `warning`, or `notice` records to `$COVE_GITHUB_ANNOTATIONS`; cove-action forwards them and copies `github-annotations.log` into the run bundle | no |
