...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  assignment creation plus worker-observed command/control cancellation and retry/replan, filterable/paginated worker and assignment inventory, worker-scoped hosted sandbox inventory, worker- and assignment-scoped event history, worker/assignment report history, worker- and assignment-scoped usage metering, non-mutating reconcile impact plans, and a reconciled operations summary; orchard still owns broader production
