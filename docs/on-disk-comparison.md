...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  dry-runs, image-preparation controls, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, metering, and GUI events, plus persisted
...
switches with capability-constrained hosted lifecycle, placement dry-runs, image-preparation controls, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, metering, and GUI events, persisted per-resource sandbox usage
