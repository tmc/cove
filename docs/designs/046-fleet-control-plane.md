...
- **API shape**, modeled on the fly.io Machines API
  ...
  /v1/sandboxes/{id}/events` for sandbox-scoped audit history and `GET
  /v1/sandboxes/{id}/metering` and `GET /v1/metering/sandboxes` for scoped
  usage records. `GET /v1/sandboxes` now filters by status, worker, image,
  namespace, offset, and limit, and returns `next_offset`, so hosted clients can
  page through only the handles they need; the Go and Python SDKs expose the
  same filters and page metadata. The OpenAI Agents Python adapter now has a `provider="cloud"`
  ...
  create/list/get/wait/lease/restart/exec/control/events/metering/delete and
