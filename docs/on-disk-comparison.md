...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  with per-namespace admission caps, queue priority, queue-deadline expiry, per-run timeout policy, bounded automatic retry, sandbox-scoped exec/control cancellation during stop/delete/restart/drain, returned canceled assignment ids, and enforced modify leases
  over fork-run assignments, OpenAI Agents Python and Go agentsandbox local/cloud
  provider switches with capability-constrained hosted lifecycle, placement
...
handles over fork-run assignments with per-namespace admission caps, queue priority, queue-deadline expiry, per-run timeout policy, bounded automatic retry, sandbox-scoped exec/control cancellation during stop/delete/restart/drain, returned canceled assignment ids, and enforced modify leases, OpenAI Agents Python and Go agentsandbox local/cloud provider
switches with capability-constrained hosted lifecycle, placement dry-runs, placement history helpers, image-preparation dry-runs with skipped-worker diagnostics and controls, operations dashboard summaries, reconcile plan/apply helpers, assignment submission, worker/assignment inventory, scoped worker/assignment observability and fleet-level sandbox metering, worker lifecycle controls, assignment cancel/retry controls, audit list/verify helpers, service-account and OIDC/SAML identity/metadata helpers, maintenance dry-runs, controls, skip diagnostics, and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, fleet-level sandbox metering, and GUI events, persisted per-resource sandbox usage
