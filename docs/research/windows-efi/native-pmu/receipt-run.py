...
root=pathlib.Path('/tmp/cove-windows-native-pmu-20260907')
...
for name in ('receipt-shim',):
 img=root/(name+'.img');assert not img.exists()
 source='/tmp/cove-winpe-receipt-20260907/receipt-base.img'
 ...
 cmd=[str(root/'probe'),'-pmu','-efi',str(img),'-graphics','virtio','-nocap','-seconds','30']
