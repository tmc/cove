...
- **orchard has a real control plane.** cove's fleet support is CLI placement,
  ...
  `cove-fleetd`/`coved -fleet-url` host-inventory boundary; orchard still owns
...
transports. The first `cove-fleetd` plus `coved -fleet-url` host-inventory
boundary is present, but tart and lume still lead in ecosystem maturity, and
