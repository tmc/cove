...
| Slice 5 — stateful fleet controller (the MIT/paid boundary) | maybe | started | L | Slice 4 | [046](046-fleet-control-plane.md) §topology | Initial `cove-fleetd` host-inventory store, durable assignment leases, `least-loaded`/`image-affinity` placement, stale-worker/expired-lease reconciliation, controller-side worker cordon/uncordon, fleet image preparation, fleet image-GC push, lifecycle-policy push, fork warm-pool quota reconciliation, agent-ready slot claim, and `coved -fleet-url` worker dial-out have landed: register, heartbeat, await-assignment, active `cove` assignment renewal, execution, and report-status are wired. Remaining: storage-budget policy fan-out and governance APIs. Controller is PAID; worker mode stays MIT. |
| Slice 6 — fleet-wide policy & GC push | maybe | started | M | Slice 5 | [046](046-fleet-control-plane.md), [031](031-vm-lifecycle.md), [040](040-storage-budget.md) | Controller now pushes image-GC and VM lifecycle-policy assignments down worker streams with skip reporting. Remaining: storage-budget/GC policy fan-out. PAID. |
| Slice 7 — top-k bin-pack scheduler + base-image affinity + fork-warm-pool | maybe | started | L | Slice 5 | [046](046-fleet-control-plane.md) §scheduler | Base-image affinity placement, slot-capped `bin-pack` placement, anti-affinity hints, the first controller image-preparation endpoint, retained top-k placement plans, durable warm-pool quota replenishment, agent-ready slot claim/Exec handoff, and claimed-slot stop cleanup have landed. Remaining work is downsize cleanup and richer pool lifecycle operations. Image-locality is cove's edge over Orchard/Daytona. PAID. |
