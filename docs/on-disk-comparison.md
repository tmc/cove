...
6. **Image-aware, drainable fleet placement.** cove is not orchard's controller,
   ...
   assignment status, skip reason, and skipped worker, so operators can see
   whether RAM-overlay capacity is ready and whether preparation or maintenance
   fan-out is currently blocked;
   ...
   before hosted work lands. Hosted assignment, sandbox, warm-pool,
   image-preparation inventory, and the aggregate controller-run timeline can
