...
| Design 041 Slice 3 — production wiring + flip default | must | spec landed (`5794c94`); impl R62 | M | Slice 2 perf clearance | [041](041-screencapturekit-migration.md) §Slice 3 | Wires the SCKit spike into the live `controlserver/Capture` path with `cgwindow`/`sckit`/`auto` selector, fallback policy, and per-VM override. Default flips to `sckit` once perf gate passes. Spec at `5794c94`. R62 impl pane in flight. Depends: Slice 2 perf clearance. |
| Design 041 Slice 4 — drop `CGWindowListCreateImage`; enforce macOS 14.0 floor | must | not started | S | Slice 3 (one release of bake time) | [041](041-screencapturekit-migration.md) §Slice 4 | Removes the deprecated capture path and bumps the documented host minimum. macOS 12/13 fallback shims retired. Per design 041, Slice 4 must not run before Slice 3 has at least one release of bake time on the default `sckit` path. Depends: Slice 3. |
...
| Cirrus secrets (`ENCRYPTED[…]`) → guest env | should | spec landed (`7f196e9`); impl pending | M | cove-action `secrets:` plumbing | [cirrus-secrets-fix-2026-05-08](../strategy/cirrus-secrets-fix-2026-05-08.md) | Closes the last engineering blocker for the Cirrus migration story before 2026-06-01: host-side `secrets:` flag + redactor on top of the already-plumbed `ExecRequest.env`. Sized M (~450 LOC, four atomic steps) at `7f196e9`. Depends: cove-action surface (shipped v0.4 at `0985377`). |
| Test coverage gap fill — control + storage internals | should | in progress (R59-R62) | M | none | R57-R62 test-gap dispatches | Targets exercised in R57-R62: `storagecensus` (`dbfacf8`, `f6e7c7c`), `storagepins` (`c5ec526`), `coved` poll scheduler (`9b7c849`). Continue across `internal/control`, `internal/controlserver`, and `internal/vmrun` to lift coverage before v0.6 GA. Sized M (per-pane single-package dispatches). |
| Deadcode + staticcheck SA4006 sweep — round 5 | should | in progress (R56-R59) | S | none | R54-R59 sweeps | Continue the U1000/SA4006 cadence shipped through R54 (`1bdd1e8`), R56 (`260e80c`, `05d0ee3`, `8d53d70`, `59be48a`), R57 (`aa1a80e`), R59 (`e3f135a`). One small sweep per release keeps the surface narrow ahead of the SCKit and storage-census churn. Sized S; no design doc. |
