...
- **orchard has a real control plane.** cove's fleet support is CLI placement,
  ...
  `cove-fleetd`/`coved -fleet-url` host-inventory, assignment-lease, and worker
  execution boundary with least-loaded and image-affinity placement; orchard
  still owns reconciliation, worker lifecycle, and richer controller concerns.
...
boundary is present with host inventory, assignment leases, and worker-side
