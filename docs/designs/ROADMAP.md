...
| Capture-latency observability surface | maybe | spec landed (`aee83c5`); Prometheus aggregation shipped (`864fb59`) | M | Slice 3 perf wiring | [042](042-capture-latency-observability.md) | Design 042 names the `capture_latency` run event, low-cardinality `coved_capture_*` Prometheus series, and insertion points through `screenshots.go`, run bundles, and `internal/coved/prometheus.go`. R106 added daemon-side latency aggregation; remaining work is wiring production capture samples from `controlserver/Capture`. |
