...
strategy in `012`, the v0.1 handoff in
`014`, the soft-reset empirical result in
`015`, and the post-integration NotebookLM refresh
in `016`. The v0.3 execution
slices are tracked in `017`. The 012, 016, and
...
|---|---|---|---|---|
| `cove up` fresh-install path-resolution fix | done | none | `roadmap-post-v0.1`, `03fb38f`, `5d7eacb`, `706cf0d`, `blocker audit` | Fixes the headline UX bug where install reported success but provisioning failed because the target VM directory was never materialized. Follow-up audit found the old no-disk smoke was caused by a disposable test wiping the real `~/.vz/vms`; resolver tracing, path-resolution tests, and the `VZ_DEBUG_INSTALL` watcher are in place. Remaining work is a clean-host smoke record, not another resolver-code blocker. |
| CDC vs fixed-offset chunking trade-off study | done | none | `002` | Settles whether content-defined chunking is worth the cost before committing the v0.2 store design. |
| `cove doctor` TCC/FDA probe | done | none | `TCC research` | Triggers/diagnoses Full Disk Access state before VirtioFS access silently fails. |
| Verify 008 codebase-cleanup status | done | none | `008` | Confirms which cleanup phases already landed and gates v0.2 work. |

## v0.2 - Linux Workstation + Foundation Cleanup

| Item | Priority | Depends on | Source | Why |
|---|---|---|---|---|
| Local content-addressed store at `~/.vz/store/` + GC | done | none | `002` | Required foundation for resumable pulls and `cove build`. |
...
| ControlServer decomposition - phases 1+2 | done | none | `008` | Moves agent and shared-folder configuration seams out of the package-main god object. |
...
| Item | Priority | Depends on | Source | Why |
|---|---|---|---|---|
| `cove build` VM execution path | done | v0.2 store + dry-run planner | `003` | Local and registry-base builds create scratch VMs, restore cache-hit layers, execute misses, persist metadata, and leave pushable image state. |
| Secrets via tmpfs (`# secret:` directive) with guest swap disabled | done | build execution | `003` | Prevents secret leakage into pushed OCI block diffs. |
| `cove build` compaction integration | done | build execution | `002`, `003` | Wires `fast`, `targeted`, and `thorough` build compaction into the pipeline before diffing and pushing images. |
| Fork-only benchmark publication | done | existing fork support | `012`, `015`, [bench result](../../bench/fork-time/results-20260427.md) | Published 132-140 ms stopped-VM fork measurements on the M4 smoke host after soft reset failed as the isolation primitive. |
| Boot-to-agent fork benchmark publication | done | existing fork support + reachable agent base image | `012`, `015`, [bench result](../../bench/fork-time/results-agent-20260430.md) | Published the product-relevant time from fork command to agent reachability on named M4 Max hardware. |
| ControlServer decomposition - phase 3 (`internal/control`) | done | v0.2 phases 1+2 | `008` | Completes the cleanup arc started in v0.2. Shipped as `cede792`. |
| OpenAI Agents SDK adapter v1 | done | fork/restore + control socket | `012`, [OpenAI example](../examples/openai-agents.md) | Proves the agent-substrate pitch with a fork-first local adapter under `adapters/openai-agents-python`. |
| OpenAI adapter release hardening | done | adapter v1 + boot-to-agent benchmark | `012`, `017` | Added live smoke instructions, package checks, and fork-first example polish before treating the adapter as a release surface. |
| Anthropic sandbox-runtime adapter | done | OpenAI adapter lessons | `012` | Expands agent integrations after the first adapter proves the shape. Shipped as `fafc32a`. |
| Curated agentkit base images | done | build execution + trademark decision | `012` | Prepares the v1.0 registry story without publishing under a blocked name. Local curated bases shipped as `92f2272`; public registry publication remains deferred. |
...
| Anthropic computer-use adapter v2 | done | agent-sandbox CLI | `022` | Adds a Go-side Anthropic Messages computer-use loop and wires it into `cove agent-sandbox run`. Design v2 update at `55a2463`; implementation at `33e5b30` (`anthropicadapter` computer-use loop) and `775537f` (`agent-sandbox` provider wiring). |
...
| GitHub Actions private executor surface | done | local image store + run metrics | `021`, [030](030-gha-executor-slice-2.md) | Ships the private GHA wrapper, action metrics, preflight commands, and cache-image reuse. Implementation: `0985377` (cove-action runner), `8bd473e` (action metrics emission), `82a0ac5` (metrics path), `7fafe40` (preflight commands), `9e6253a` (Slice 2 local image cache restore); accompanying executor docs at `19804c7`. |
...
| Soft-reset destructive probe matrix + orchestrator | done | none | `015` | Adds network, memory, process-table, filesystem-attribute probes and a run-all orchestrator. Shipped at `8c7d54b`, `a51c544`, `31017ca`, `0ed3b70`, `ad9dc16`, `7030657`, and `6773695`. |
...
| Network policy, logs, and forwarding | done | control socket + guest agent | `001`, [034](034-fleet-slice-1.md) | Adds named policy audit logs, `cove network logs`, `cove forward` reverse direction, and UDP support. Shipped at `1ac32f9`, `6fd5bc5`, `6e6fa18`, `22e6c43`, and `235b678`. |
...
| v0.4 readiness and integration matrix | done | shipped v0.4 surfaces | `release audit` | Adds v0.4 release notes draft, integration matrix, cross-reference pass, and readiness audit. Shipped at `96706d2`, `7852177`, `c29eae3`, and `14cabac`; this closeout updates the final docs. |
...
| Guest → host artifact copy-out (`cove runs export --include-guest`, cove-action `artifacts`) | should | shipped (`9b88e8e` plus cove-action convention) | M | guest agent + runs bundle | `Cirrus migration readiness 2026-05-08` §Partial | `cove runs export --include-guest` and cove-action `artifacts` copy guest artifacts into the run bundle so CI scripts do not need trailing hand-written copy commands. |
| Cirrus secrets (`ENCRYPTED[…]`) → guest env | should | Slices 1-2 shipped (`fe99629`, `ab7f159`) | M | cove-action `secrets:` plumbing | `cirrus-secrets-fix-2026-05-08` | Closes the last engineering blocker for the Cirrus migration story before 2026-06-01: host-side `secrets:` flag + redactor on top of the already-plumbed `ExecRequest.env`. Spec at `7f196e9`; Slice 1 (metrics redaction) shipped at `fe99629`; Slice 2 (cove-action `secrets:` input → `--secret-env`) shipped at `ab7f159`. Depends: cove-action surface (shipped v0.4 at `0985377`). |
...
| Cirrus shutdown 2026-06-01 closeout | should | blocked until 2026-06-01 | v0.6 Cirrus secrets surface + real post-shutdown docs pass | `readiness audit`, [migration checklist](../migrations/from-cirrus-checklist.md) | After 2026-06-01, verify migration docs, benchmark notes, and landing pages against the actual shutdown state, then remove rollback language that only applied while Cirrus still existed. Do not pre-close this before the date passes. |
| macOS 15 host audit | should | blocked: needs operator host evidence | v0.6 macOS 14 floor + TCC-granted SCKit bench host | `macOS floor audit`, [041 preflight](041-slice4-preflight.md) | Current local docs establish the v0.6 macOS 14 floor and do not justify a v0.7 floor change. Before deciding whether v0.7 remains macOS 14 or bumps to 15, record a macOS 15 host run of `cove doctor sckit-preauth`, live `internal/sckit` capture tests with a visible GUI VM, `cove doctor sckit-spike -n 30 -threshold 50ms -title-prefix cove`, and any Virtualization.framework behavior differences observed during that run. |
| Privacy gate review | should | planned | product/legal decision | `privacy-gated gaps` | Decide when the private cove repo can flip public, and list prerequisites for public action, image catalog, registry, and signing channels. |
...
see `017` for files, gates, and docs updates.
...
- `cove` is not clean for public software/registry branding based on the preliminary USPTO search. Do not take a public v1 registry out under this name without trademark counsel or a rename plan. See `docs/research/trademark-cove.md`.
...
- **2026-05-02**: design [025](025-cove-action-security.md) cove-action security architecture landed at `1db4830` (411 LOC). Threat model + token lifecycle + isolation invariants for the v0.4 cove-action GHA wrapper. Clears the security-gate prerequisite on design `021` Slice 1 implementation. Per user 2026-05-02: Slice 1 still v0.4-targeted; cove repo stays private; design [024](024-cove-runner-images.md) Slice 3 deferred indefinitely.
...
- **2026-05-02**: Trimmed Buildkite track from v0.4 design `021`; v0.4 CI work now covers GHA + GitLab only.
