...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  assignment cancellation and retry/replan, filterable/paginated worker and assignment inventory, worker-scoped event history, non-mutating reconcile impact plans, and a reconciled operations summary; orchard still owns broader production
...
expired-lease reconciliation with non-mutating impact plans, audited direct assignment cancellation and retry/replan, filterable/paginated worker and assignment inventory plus worker-scoped event history, controller-side worker cordon/quarantine/evacuate/drain/decommission
