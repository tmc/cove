...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  lifecycle-policy push with retained history, storage budget/prune push with retained history, and worker execution
...
operations summary, manifest-bundle-backed digest-aware fleet image preparation with retained history, fleet image-GC push with retained history, lifecycle-policy push with retained history, storage budget/prune
push with retained history, and worker-side `cove` assignment execution plus
