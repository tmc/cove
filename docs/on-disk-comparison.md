...
6. **Image-aware, drainable fleet placement.** cove is not orchard's controller,
   ...
   before hosted work lands. Hosted sandbox and warm-pool inventory can filter
   by manifest digest, digest ref, platform, image ref, and required
   capability, so operators can find exactly which live handles and ready
   capacity depend on an immutable image or RAM-overlay-class worker
   capability.
