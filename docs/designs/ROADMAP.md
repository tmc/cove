...
| Apple App Sandbox proof lane | should | proof harness + host status landed; packaging proof queued | signed build + packaging proof | [045](045-app-sandbox.md) | Explore whether cove can run with `com.apple.security.app-sandbox` without weakening the operator model. The opt-in harness now proves a raw ad-hoc sandboxed CLI can be signed but traps before `--version`, `help`, and `list`; `security status` and `doctor host` report host App Sandbox state separately from guest containment. Next work is `.app`/worker architecture proof, not a simple entitlement flip. |
