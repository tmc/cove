...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  reconciliation, worker cordon, digest-aware fleet image preparation with dry-run skip diagnostics and retained history, fleet image-GC push with retained history,
  ...
  dry-runs, image-preparation dry-runs with skipped-worker diagnostics and controls, maintenance controls and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, metering, and GUI events, plus persisted
...
operations summary, aggregate retained controller-run feed, manifest-bundle-backed digest-aware fleet image preparation with dry-run skip diagnostics and retained history, fleet image-GC push with retained history, lifecycle-policy push with retained history, storage budget/prune
...
switches with capability-constrained hosted lifecycle, placement dry-runs, image-preparation dry-runs with skipped-worker diagnostics and controls, maintenance controls and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, metering, and GUI events, persisted per-resource sandbox usage
