...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  with per-namespace admission caps, queue priority, queue-deadline expiry, per-run timeout policy, bounded automatic retry, and enforced modify leases
  ...
  assignment creation plus worker-observed cancellation and retry/replan, filterable/paginated worker and assignment inventory, worker-scoped hosted sandbox inventory, worker- and assignment-scoped event history, worker/assignment report history, worker- and assignment-scoped usage metering, non-mutating reconcile impact plans, and a reconciled operations summary; orchard still owns broader production
...
expired-lease reconciliation with non-mutating impact plans, audited direct assignment creation plus worker-observed cancellation and retry/replan, filterable/paginated worker and assignment inventory, worker-scoped hosted sandbox inventory, worker- and assignment-scoped event history, worker/assignment report history, and worker- and assignment-scoped usage metering, controller-side worker cordon/quarantine/evacuate/drain/decommission
...
handles over fork-run assignments with per-namespace admission caps, queue priority, queue-deadline expiry, per-run timeout policy, bounded automatic retry, and enforced modify leases, OpenAI Agents Python and Go agentsandbox local/cloud provider
