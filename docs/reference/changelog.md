...
- Server-side `cove shell` control-socket commands: `agent-exec-attach`, `agent-exec-resize`, `agent-exec-signal` on the per-VM control socket. Reuses existing `control.token` auth and the long-lived stream dispatch arm. No proto bump; stdin frames are decoded and discarded (Slice 3 wires bidi). Standalone `cove shell <vm>` client is the next slice.
