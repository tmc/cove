...
switches with capability-constrained hosted lifecycle, placement dry-runs, placement history helpers, image-preparation dry-runs with skipped-worker diagnostics and controls, operations dashboard summaries, reconcile plan/apply helpers, worker/assignment inventory, scoped worker/assignment observability, worker lifecycle controls, assignment cancel/retry controls, audit list/verify helpers, service-account and OIDC/SAML identity/metadata helpers, maintenance dry-runs, controls, skip diagnostics, and run history, warm-pool lifecycle controls, list filters, pagination, sandbox event/report history, metering, and GUI events, persisted per-resource sandbox usage
