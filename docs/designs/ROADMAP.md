...
| Apple App Sandbox proof lane | should | queued | signed build + packaging proof | [045](045-app-sandbox.md) | Explore whether cove can run with `com.apple.security.app-sandbox` without weakening the operator model. First proof showed a raw ad-hoc sandboxed CLI can be signed but traps before `--version`, so the queued work is a reversible harness plus `.app`/worker architecture proof, not a simple entitlement flip. |
