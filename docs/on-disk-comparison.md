...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  and worker execution boundary with least-loaded, image-affinity, and
  slot-capped bin-pack placement plus anti-affinity hints, retained placement
  plans, and first warm-pool quota replenishment plus agent-ready slot claim
...
preparation, fleet image-GC push, and worker-side `cove` assignment execution plus
...
claim into same-worker guest `Exec` with stop cleanup, but tart and lume still
