...
6. **Image-aware, drainable fleet placement.** cove is not orchard's controller,
   ...
   heartbeats now report image source manifest digests, and assignment plus
   placement-plan requests can require capabilities while assignment,
   placement-plan, warm-pool, sandbox, and image-preparation requests can carry
   or resolve the selected digest from an offline
...
- **orchard has the more mature control plane.** cove's fleet support is CLI
  ...
  boundary with required-capability matching, least-loaded, image-affinity, and
  slot-capped bin-pack placement plus anti-affinity hints,
...
required-capability matching and least-loaded/digest-exact image-affinity/bin-pack placement with anti-affinity hints and
