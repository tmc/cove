...
- **API shape**, modeled on the fly.io Machines API
  ...
  readiness through `cove shell`, waits on terminal status, and stops/deletes by
  canceling pending work or queueing same-worker stop cleanup. Remaining work is
  restart/start-from-stopped behavior and the SDK/provider surface. The bar to
...
| 8 | Hosted API + SDK provider abstraction + metering | REST `/v1/sandboxes` surface; create/list/get/delete/stop/wait scaffold has started; `provider=local\|cloud` SDK; per-resource metering; BYO-LLM-key | PAID (hosted) | Slice 7 | Use case C; competes with Daytona/Cua on the macOS wedge. |
