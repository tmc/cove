...
var cliDocSpecs = []cliDocSpec{
	...
	{Name: "build", Summary: "Build VM images from vzscript steps with content-addressed cache keys. Base can be a registry reference or local VM directory.", Usage: func() string {
		...
		{Name: "--cache-from", Argument: "<ref>", Summary: "Import an OCI build cache before cache hit evaluation.", Repeatable: true},
		{Name: "--cache-to", Argument: "<ref>", Summary: "Export build cache entries after a successful build.", Repeatable: true},
	...
	{Name: "pull", Summary: "Validate or pull an OCI VM image.", Usage: func() string {
		...
		{Name: "--manifest-bundle", Argument: "<dir>", Summary: "Verified offline manifest bundle instead of fetching the registry."},
		...
		"cove pull ghcr.io/me/dev-vm:v1 --dry-run --fetch-manifest --index-out index.json",
		"cove pull ghcr.io/me/dev-vm:v1 --dry-run --fetch-manifest --all-platforms --manifest-dir manifests",
		"cove pull ghcr.io/me/dev-vm:v1 --dry-run --manifest-bundle manifests --platform linux/arm64 --json",
