...
| Capture-latency observability surface | maybe | spec landed | M | Slice 3 perf wiring | [042](042-capture-latency-observability.md) | Slice 2's "p50/p95 measured" gate currently lives in the spike binary. Design 042 names the `capture_latency` run event, low-cardinality `coved_capture_*` Prometheus series, and insertion points through `screenshots.go`, run bundles, and `internal/coved/prometheus.go`. Depends: Slice 3 production wiring lands `controlserver/Capture` instrumentation point. |
