...
volatile u64 emu_count, emu_reason, emu_pcs[64], emu_gp[32], emu_ranges[64], emu_nrange, emu_stop[4];
...
static EFI_STATUS trace_entry(EFI_HANDLE child) {
    ...
    u64 stop_rva=STOP_RVA;
    if (stop_rva>size-4 || (stop_rva&3)) return EFI_UNSUPPORTED;
    u32 *stop=(u32*)(base+stop_rva);
    int safe_stop=0;
    for (u64 i=0;i<emu_nrange;i++) if ((u64)stop>=emu_ranges[i*2] && (u64)stop<=emu_ranges[i*2+1]) safe_stop=1;
    if (!safe_stop) return EFI_UNSUPPORTED;
    emu_stop[0]=(u64)stop; emu_stop[1]=*stop;
    __asm__ volatile("mrs %0, cntvct_el0" : "=r"(emu_stop[2]));
    emit("post-read breakpoint "); emit_u64((u64)stop,1); emit(" original instruction "); emit_u64(*stop,1); emit("\n"); flush_report();
    *stop=0xd42000e0; sync_code(stop);
    u64 tpl=((u64(*)(u64))BS->RaiseTPL)(31); ((void(*)(u64))BS->RestoreTPL)(tpl);
    u64 result=entry_call((void*)BS->StartImage,child);
    *stop=(u32)emu_stop[1]; sync_code(stop);
    __asm__ volatile("mrs %0, cntvct_el0" : "=r"(emu_stop[3]));
    ...
    emit("counter interval "); emit_u64(emu_stop[2],1); emit(" to "); emit_u64(emu_stop[3],1); emit("\n");
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
    ...
    emit("NOTE: original PMCCNTR executes; only the following instruction has a one-shot BRK.\n");
