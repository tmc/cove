...
| Slice 5 — stateful fleet controller (the MIT/paid boundary) | maybe | started | L | Slice 4 | [046](046-fleet-control-plane.md) §topology | Initial `cove-fleetd` host-inventory store, durable assignment leases, and `coved -fleet-url` worker dial-out have landed: register, heartbeat, await-assignment, `cove` assignment execution, and report-status are wired. Remaining: scheduler placement and reconciliation. Controller is PAID; worker mode stays MIT. |
