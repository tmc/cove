...
extern u64 read_dfr0(void), read_cntfrq(void), read_cntvct(void);
volatile u64 trap_state[8];
static u64 counter_read(const char *name, u64 (*fn)(void), int *ok) {
    u64 old_vbar, old_daif;
    trap_state[0]=0; trap_state[1]=0;
    ...
    *ok=trap_state[0]==0;
    emit(name); emit(" instruction "); emit_u64((u64)(void*)fn,1);
    if (*ok) { emit(" value "); emit_u64(value,1); emit(" (read completed)\n"); }
    else { emit(" FAULT ELR "); emit_u64(trap_state[0],1); emit(" ESR "); emit_u64(trap_state[1],1); emit("\n"); }
    flush_report();
    return value;
}
static void probe_exception(void) {
    ...
    emit("CurrentEL "); emit_u64(el,1); emit(" SPSel "); emit_u64(sel,1); emit("\n");
    if (el!=4 || sel!=1) { emit("unsupported exception context\n"); return; }
    int dfr_ok,frq_ok,c0_ok,c1_ok,c2_ok;
    u64 dfr=counter_read("ID_AA64DFR0_EL1",read_dfr0,&dfr_ok);
    if (dfr_ok) { emit("PMUVer field "); emit_u64((dfr>>8)&15,1); emit("\n"); }
    u64 frq=counter_read("CNTFRQ_EL0",read_cntfrq,&frq_ok);
    u64 c0=counter_read("CNTVCT_EL0 first",read_cntvct,&c0_ok);
    for (volatile u64 i=0;i<10000;i++) {}
    u64 c1=counter_read("CNTVCT_EL0 second",read_cntvct,&c1_ok);
    for (volatile u64 i=0;i<10000;i++) {}
    u64 c2=counter_read("CNTVCT_EL0 third",read_cntvct,&c2_ok);
    emit(frq_ok && frq && c0_ok && c1_ok && c2_ok && c1>=c0 && c2>=c1 && c2>c0 ? "VIRTUAL COUNTER ADVANCE PASS\n" : "VIRTUAL COUNTER INCONCLUSIVE\n");
    flush_report();
}
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
    ...
    emit("cove guest counter capability control\n");
    emit("=======================================================\n");
