...
6. **Image-aware, drainable fleet placement.** cove is not orchard's controller,
   ...
   requests can carry the selected digest so stale mutable refs are rejected or
   force-refreshed before hosted work lands.
...
manifest-bundle and controller-side source-manifest exactness checks for mutable fork refs, and concurrent
...
summary, digest-aware fleet image preparation, hosted sandbox worker drain, fleet image-GC push, lifecycle-policy push, storage budget/prune
