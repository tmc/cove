...
- Anthropic computer-use bridge under `adapters/anthropic-bridge/`. Single-file Python helper that translates Messages API `tool_use` blocks into cove control-socket commands; no SDK dependency. See `docs/examples/anthropic-computer-use.md`.
- Gemini computer-use bridge under `adapters/google-bridge/`. Translates Gemini `function_call` parts into cove control-socket commands and feeds the next screenshot back as a `function_response`. See `docs/examples/gemini-computer-use.md`.
- `cove run -fork-from <image-ref> -ephemeral`: fork-from accepts a local image ref in addition to a VM name. VM-name wins on collision. `-ephemeral` drops the existing `.ephemeral` sentinel so `cove gc` sweeps the materialized child on stop.
...
- `cove shell <vm>`: Docker-shaped standalone exec client. `cove shell <vm>` opens an interactive `bash -l` against a running VM; `cove shell <vm> -- argv...` runs a one-shot command and propagates stdout/stderr/exit-code. SIGWINCH forwards to `agent-exec-resize`; SIGINT detaches the main cove handler so Ctrl-C goes to the guest. Stdin remains read-only in this slice -- bidirectional stdin and the `ExecAttach` proto RPC are deferred to design 023 Slice 3 in v0.3.
- Server-side `cove shell` control-socket commands: `agent-exec-attach`, `agent-exec-resize`, `agent-exec-signal` on the per-VM control socket. Reuses existing `control.token` auth and the long-lived stream dispatch arm. No proto bump; stdin frames are decoded and discarded (Slice 3 wires bidi).
