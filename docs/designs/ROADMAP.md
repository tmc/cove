...
| Test coverage gap fill — control + storage internals | should | saturated; local gates green (`cd14f9e`); main CI green on cove-hermes-v10 | M | none | R57-R111 test-gap dispatches | Coverage sweeps filled small/medium branches across storage, controlclient, controlserver, windows, runs, sckit, ociimage, and related daemon surfaces. Final cleanup at `cd14f9e` makes `go build ./...`, `go vet ./...`, and `go test ./...` pass on local main; `1c31744`/`0d5b7af` move the private PR workflow onto the cove runner with setup-go cache disabled. Further coverage work should start from a fresh low-watermark audit rather than this v0.6 sweep row. |
