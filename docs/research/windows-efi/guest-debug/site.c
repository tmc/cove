...
static EFI_STATUS trace_entry(EFI_HANDLE child) {
    ...
    *entry=original; sync_code(entry);
    ...
    for (u64 i=0;i<9;i++) { emit("x"); emit_u64(i,0); emit(" "); emit_u64(entry_trap[6+i],1); emit("\n"); }
    emit(result==1 && entry_trap[0]==(u64)entry && entry_trap[1]==0xf2000007 ? "SITE BREAKPOINT PASS\n" : "SITE BREAKPOINT INCONCLUSIVE\n");
...
EFI_STATUS efi_main(EFI_HANDLE image, SYSTEM_TABLE *st){
    ...
    emit("NOTE: this one-shot breakpoint is instrumented execution, not an original-stall sample.\n");
