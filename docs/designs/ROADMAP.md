...
| Test coverage gap fill — control + storage internals | should | saturated; local gates green (`cd14f9e`) | M | none | R57-R111 test-gap dispatches | Coverage sweeps filled small/medium branches across storage, controlclient, controlserver, windows, runs, sckit, ociimage, and related daemon surfaces. Final cleanup at `cd14f9e` makes `go build ./...`, `go vet ./...`, and `go test ./...` pass on local main. Further coverage work should start from a fresh low-watermark audit rather than this v0.6 sweep row. |
...
- **2026-05-11** (R111): Capture-latency production wiring shipped at `ecbf3bb`; final local gate cleanup at `cd14f9e` makes `go build ./...`, `go vet ./...`, and `go test ./...` pass on main. GitHub-hosted macOS Actions are still blocked before job start by account billing/spending-limit settings, not code.
