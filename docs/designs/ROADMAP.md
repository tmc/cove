...
| Guest → host artifact copy-out (`cove runs export --include-guest`) | should | shipped (`9b88e8e`) | M | guest agent + runs bundle | [Cirrus migration readiness 2026-05-08](../strategy/cirrus-migration-readiness-2026-05-08.md) §Partial | `cove runs export --include-guest` copies guest artifacts into the run export bundle so CI scripts do not need a trailing hand-written `cove ctl cp`. Further scope beyond the agent stream remains a deferred follow-up per blocker audit `4a3029d`. |
